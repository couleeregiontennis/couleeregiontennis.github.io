...
async function main() {
  try {
    ...
    Object.values(teams).forEach(team => {
      ...
      // Escape all variables before interpolation into HTML
      const safeNight = escapeHTML(night);
      const safeTeamNumber = escapeHTML(teamNumber);
      const safeTeamNameHtml = escapeHTML(teamName);
      const safeCaptName = escapeHTML(captain.name);
      const safeCaptPhone = escapeHTML(captain.phone);
      const safeCoCaptName = coCaptain ? escapeHTML(coCaptain.name) : '';
      const safeCoCaptPhone = coCaptain ? escapeHTML(coCaptain.phone) : '';
      const safeCoordName = escapeHTML(nightCoordinator.name);
      const safeCoordPhone = escapeHTML(nightCoordinator.phone);
          ...
          <table width="100%" cellpadding="0" cellspacing="0" style="max-width: 700px; margin: auto;">
            <tr>
              <td style="padding: 20px;">
                <h2 style="color: #004080; margin-bottom: 5px;">Hello LTTA ${safeNight} Team ${safeTeamNumber} ${safeTeamNameHtml}:</h2>
                ...
                <div style="background:#f5f8fb; border-radius:8px; padding:18px 16px; margin:28px 0;">
                  <p><strong>League Start:</strong> ${safeNight === 'Tues' ? 'Tuesday, June 3' : 'Wednesday, June 4'} at 5:30 PM or 7:00 PM<br>
