  ...
  try {
    ...
    (scheduleData.schedule || []).forEach(match => {
      ...
      const icsLink = match.ics
        ? `<a href="${sanitizeUrl(match.ics)}?v=2026.9" download="LTTA-Match-Week${escapeHTML(match.week)}.ics" title="Add to calendar">📅</a>`
        : '';
    ...
    if (teamIcsLink) {
      ...
      teamIcsLink.href = sanitizeUrl(`${baseIcsUrl}?v=2026.9`);
    }
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (team && day) {
    ...
    loadTeamData(`../teams/${day}/schedules/${team}.json?v=2026.9`, `../teams/${day}/rosters/${team}.json?v=2026.9`);
  }
});
