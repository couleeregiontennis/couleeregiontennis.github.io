...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Parse request body
    const { year, night, startDate, teamIds, seed, preview = false, replaceIfExists = false, locationId } = await req.json();
    ...
    // Fetch court groups for the location
    const locationIdToUse = locationId || await getDefaultLocationId(supabaseClient);

    const { data: courtGroupsData, error: courtGroupsError } = await supabaseClient
      .from('court_group')
      .select('group_name, court_numbers')
      .eq('location_id', locationIdToUse)
      .order('group_name');

    if (courtGroupsError || !courtGroupsData || courtGroupsData.length === 0) {
      throw new Error('No court groups configured for this location');
    }

    // Convert court groups to the format expected by scheduler
    const courtGroups = courtGroupsData.map(g => g.group_name);
    ...
    const schedule = generateRoundRobinSchedule(teamsData, startDate, seed, courtGroups);
...
// Get default location for a night (can be enhanced per-night defaults)
async function getDefaultLocationId(supabase: any): Promise<string> {
  // For now, return Green Island as default
  // In the future, this could be configured per-night
...
function generateRoundRobinSchedule(teams: any[], startDate: string, seed: string | undefined, courtGroups: string[]) {
  ...
  return assignSlots(roundsData, teamCourtUsage, teamTimeUsage, courtGroups);
}

function assignSlots(rounds: any[], teamCourtUsage: Record<string, any>, teamTimeUsage: Record<string, any>, courtGroups: string[]) {
  // Create slots dynamically based on court groups from database
